...
public class DrivingRouteManager extends AbstractRepository {
    ...
    private void deleteByIdUnsafe(int i) {
        getDatabase().delete(DrivingRoute.TABLE_NAME, whereWrapper("%s=?", "_id"), whereArgsWrapper(Integer.valueOf(i)));
    }

    private void deleteByIds(final List<Integer> list) {
        if (list == null || list.isEmpty()) {
            return;
        }
        transactionExecute(new Runnable() { // from class: com.xiaopeng.ota.remind.-$$Lambda$DrivingRouteManager$QXvCHj_UAShXM50yblCeKcCme4k
            @Override // java.lang.Runnable
            public final void run() {
                DrivingRouteManager.this.lambda$deleteByIds$0$DrivingRouteManager(list);
            }
        });
    }

    public /* synthetic */ void lambda$deleteByIds$0$DrivingRouteManager(List list) {
        Iterator it = list.iterator();
        while (it.hasNext()) {
            Integer num = (Integer) it.next();
            if (num != null) {
                deleteByIdUnsafe(num.intValue());
            }
        }
    }
}
