...
public class StatEvent implements IStatEvent {
    ...
    public static String getModuleNameFromEvent(String str) {
        String[] split;
        return (TextUtils.isEmpty(str) || (split = str.split(OTAConstants.LINKER_UNDER_LINE)) == null || split.length <= 0) ? "" : split[0];
    }
    ...
    public static String getMCUVer() {
        ...
        if (TextUtils.isEmpty(sMcuVersion)) {
            sMcuVersion = SystemProperties.get(Config.MCU_VERSION_PROP);
        }
        return sMcuVersion;
    }
