...
public class AiPromptService extends PromptService {
    ...
    */
    ...
    private void ipcSend(String str) {
        sendApiRouteData(IpcConfig.MessageCenterConfig.IPC_ID_APP_PUSH_MESSAGE, str);
    ...
    private void closeMessage(String str) {
        ...
        sendApiRouteData(11014, str);
        LogUtils.d(TAG, "Close message: %s", str);
    }

    private void sendApiRouteData(int i, String str) {
        try {
            JsonObject jsonObject = new JsonObject();
            jsonObject.addProperty(IpcConfig.IPCKey.STRING_MSG, str);
            ApiRouter.route(new Uri.Builder().authority("com.xiaopeng.aiassistant.IpcRouterService").path("onReceiverData").appendQueryParameter(OTAConstants.JsonKey.ID, String.valueOf(i)).appendQueryParameter("bundle", jsonObject.toString()).build());
        } catch (Exception e) {
            LogUtils.e(TAG, e, "Send ipc message fail");
        }
    }

    @Subscribe(threadMode = ThreadMode.BACKGROUND)
    public void onEvent(IpcRouterEvent ipcRouterEvent) {
        LogUtils.d(TAG, "onEvent mMsgID=%d", Integer.valueOf(ipcRouterEvent.getId()));
        try {
            if (ipcRouterEvent.getId() == 10011) {
                ...
                LogUtils.d(TAG, "Bundle packageName: %s, payload data: %s", string, string2);
                if (this.AI_ASSISTANT_PKG_NAME.equals(string)) {
                    handlePayload(string2);
                }
            }
        } catch (JSONException e) {
            LogUtils.e(TAG, e, "Handle ipc route event fail");
        }
    }

    private void handlePayload(String str) {
        AiContent parse = AiContent.parse(str);
