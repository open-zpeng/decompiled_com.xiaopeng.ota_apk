...
public class MainV2Fragment extends AbstractMainFragment {
    ...
    public /* synthetic */ void lambda$initViews$4$MainV2Fragment(CompoundButton compoundButton, boolean z) {
        changeAutoUpgrade(this.mSAutoUpgrade, z);
    }
    ...
    public List<Pair<String, String>> getMainInfoData(String str) {
        ...
        if (!TextUtils.isEmpty(vinCode)) {
            arrayList.add(new Pair(ResourceUtils.getString(R.string.main_vin_prefix), vinCode));
        } else {
        ...
        if (activeWaitingCampaign != null) {
            ...
            if (c == 0) {
                ...
                if (ScheduleHelper.getInstance().isSetScheduleTime()) {
                    string = StringUtils.format(Constants.ConfigKey.MAIN_BUTTON_HAS_SCHEDULED_WITH_TIME, ScheduleHelper.getInstance().getScheduleTime());
                } else if (OTAManager.getPreferencesManager().supportAutoUpgrade()) {
                    string = ConfigHelper.getString(Constants.ConfigKey.MAIN_BUTTON_UPGRADE_AUTO);
                } else {
                    string = ConfigHelper.getString(Constants.ConfigKey.MAIN_BUTTON_UPGRADE_TO_NEW_VERSION);
                }
            } else {
                string = ConfigHelper.getString(Constants.ConfigKey.MAIN_NEW_VERSION_PREFIX) + " " + VersionUtils.getSimpleVersion(activeWaitingCampaign.getReleaseVersion());
            ...
            this.mBtnNewVersion.setText(string);
